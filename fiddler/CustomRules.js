...
class Handlers
{
    ...
    */
    ...
    RulesString("&Whoami", true)
    RulesStringValue(0, "liushoukai", "liushoukai")
    RulesStringValue(1, "liuyixing", "liuyixing")
    RulesStringValue(2, "chenweifeng", "chenweifeng")
    RulesStringValue(3, "menglexing", "menglexing")
    public static var sWhoami: String = null;
        
    RulesString("&hosts", true)
    RulesStringValue(0, "hosts_dev", 0) // 开发环境
    RulesStringValue(1, "hosts_pro", 1) // 预发环境
    RulesStringValue(2, "hosts_bei", 2) // 生产环境
    public static var hosts: String = 0;
    ...
    static function OnBeforeRequest(oSession: Session) {
        ...
        // 设置会话UI样式
        if (oSession.uriContains("up24")) {	
            oSession["ui-color"] = "brown";	
            oSession["ui-italic"] = "true";
        }
        
        // 添加请求头信息
        if ((null != sWhoami) && (oSession.uriContains("up24.com"))) {
            oSession.oRequest['whoami'] = sWhoami;
        }
        
        // 公用HOSTS配置
        var commonHosts = new Array(
            "127.0.0.1 www.test.com",
        )；
        for (var i=0; i<commonHosts.length; i++) {
            var arr = commonHosts[i].split(" ");               
        ...
        if (null != hosts) {
            ...
            // 开发环境
            hostsList[0] = new Array(
                "127.0.0.1 your.project.com",
            );
            // 预发环境
            hostsList[1] = new Array(
                "127.0.0.1 your.project.com",
            );
            // 生产环境
            hostsList[2] = new Array(
                "127.0.0.1 your.project.com",
            );
            
            for (var i=0; i<hostsList[hosts].length; i++) {
                var arr = hostsList[hosts][i].split(" ");               
                if (oSession.HostnameIs(arr[1])) {
                    oSession["x-overridehost"] = arr[0];
                    FiddlerObject.log("set domain [" + arr[1] + "] to ip [" + arr[0] + "]");
                }
            }
        }
...
*/
    ...
    static function Main() {
        ...
        FiddlerObject.UI.RegisterCustomHotkey(HotkeyModifiers.Windows, Keys.G, "screenshot");
    }
